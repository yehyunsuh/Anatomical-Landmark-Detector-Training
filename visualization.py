...
def overlay_gt_masks(images, masks, pred_coords, gt_coords, epoch, total_epoch, idx):
    ...
    for b in range(images.shape[0]):
        ...
        if epoch % 10 == 0 or epoch == total_epoch - 1:
            ...
            cv2.imwrite(
                f"visualization/Epoch{epoch}/Epoch{epoch}_Batch{idx}_overlay_gt.png", overlay
            )
    ...
    for b in range(images.shape[0]):
        ...
        for c in range(C):
            ...
            if epoch % 10 == 0 or epoch == total_epoch - 1:
                os.makedirs(f"visualization/Epoch{epoch}", exist_ok=True)
                cv2.imwrite(
                    f"visualization/Epoch{epoch}/Epoch{epoch}_Batch{idx}_Landmark{c}.png", overlay
                )
...
def overlay_pred_coords(images, pred_coords, gt_coords, epoch, total_epoch, idx):
    """
    ...
    for b in range(images.shape[0]):
        ...
        if epoch % 10 == 0 or epoch == total_epoch - 1:
            os.makedirs(f"visualization/Epoch{epoch}", exist_ok=True)
            cv2.imwrite(
                f"visualization/Epoch{epoch}/Epoch{epoch}_Batch{idx}_overlay_pred.png", img
            )
        cv2.imwrite(f"visualization/Batch{idx}_overlay_pred.png", img)

        return img


def create_gif(gt_mask_w_coords_image_list, pred_mask_w_coords_image_list_list, coords_image_list):
    """
    ...
    imageio.mimsave("train_results/gt_mask_with_coords.gif", gt_mask_frames, fps=10)
    for i, frames in enumerate(pred_mask_frames):
        imageio.mimsave(f"train_results/pred_mask_with_coords_{i}.gif", frames, fps=10)
    imageio.mimsave("train_results/pred_coords_only.gif", coords_frames, fps=10)
